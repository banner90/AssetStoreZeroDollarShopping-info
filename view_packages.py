...
class PackageViewerApp:
        ...
        def run():
            try:
                ...
                def cb(i, total, pid, name, ok):
                    msg = f"[{'OK' if ok else 'SKIP'}] ({i}/{total}) {pid} {name}"
                    self.root.after(0, lambda m=msg: self._log(m))

                success, failed = run_fetch(limit=limit, progress_callback=cb)
                self.root.after(0, lambda: self._fetch_done(success, failed))
            except Exception as e:
                err = str(e)
    ...
    def _fetch_done(self, success: int, failed: int):
        self.fetch_running = False
        self.fetch_btn.config(state=tk.NORMAL)
        self.fetch_status.config(text=f"完成: 成功={success}, 失败={failed}")
        self._log(f"\n[DONE] 成功={success}, 失败={failed}, 元数据库目录={METADATA_DIR}")
